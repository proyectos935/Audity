...
export class MultitrackPlayer {
  ...
  private activeEQPreset: string = 'none';

  constructor() {
    this.activeNodes = [];
  }

  public setEQPreset(presetId: string, tracks?: Track[], audioBufferMap?: Map<string, AudioBuffer>, maxDuration?: number) {
    this.activeEQPreset = presetId || 'none';
    if (this.isPlaying && tracks && audioBufferMap && maxDuration !== undefined) {
      const cur = this.getCurrentTime();
      this.stopNodesOnly();
      this.currentTime = cur;
      this.play(tracks, audioBufferMap, maxDuration, presetId);
    }
  }
  ...
  public seek(time: number, tracks: Track[], audioBufferMap: Map<string, AudioBuffer>, maxDuration: number, eqPreset?: string) {
    ...
    if (wasPlaying) {
      this.play(tracks, audioBufferMap, maxDuration, eqPreset);
    }
  }

  public play(tracks: Track[], audioBufferMap: Map<string, AudioBuffer>, maxDuration: number, eqPreset?: string) {
    if (eqPreset !== undefined) {
      this.activeEQPreset = eqPreset;
    }
    ...
    tracks.forEach(track => {
      ...
      track.clips.forEach(clip => {
        ...
        const eqToUse = this.activeEQPreset || 'none';
        const { outputNode, detuneCents } = applyEQPresetNodes(ctx, gainNode, eqToUse);

        if (detuneCents && sourceNode.detune) {
