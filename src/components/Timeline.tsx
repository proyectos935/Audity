...
export const Timeline: React.FC<TimelineProps> = ({
  ...
  // Calculate total visible timeline duration and ticks
  const maxTimeRange = Math.max(30, totalDuration > 0 ? totalDuration + 10 : 30, playheadTime + 5);
  const timelineWidthPx = Math.max(800, maxTimeRange * zoom);
  const stepSeconds = zoom >= 50 ? 5 : zoom >= 20 ? 10 : 30;

  const ticks: number[] = [];
  for (let t = 0; t <= maxTimeRange; t += stepSeconds) {
  ...
  const updateSeekFromEvent = (e: MouseEvent | React.MouseEvent) => {
    ...
    const newTime = Math.max(0, Math.min(clickX / zoom, maxTimeRange));
    onSeek(newTime);
  };
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 z-30 flex bg-[#1D006B] border-b-2 border-[#0A018A] shadow-md min-w-max">
        ...
        <div
          ...
          className="flex-shrink-0 h-9 bg-[#0A018A]/60 relative cursor-pointer group/ruler"
      ...
      <div className="flex-1 relative flex flex-col min-h-[300px] min-w-max">
