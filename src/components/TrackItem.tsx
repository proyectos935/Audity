...
export const TrackItem: React.FC<TrackItemProps> = ({
  ...
  const handleTrackLaneMouseDown = (e: React.MouseEvent) => {
    const target = e.target as HTMLElement;
    if (target.closest('button') || target.closest('input')) {
      return;
    }
    if (trackLaneRef.current) {
      const rect = trackLaneRef.current.getBoundingClientRect();
      const clickX = e.clientX - rect.left;
      const targetTime = Math.max(0, Math.min(clickX / zoom, maxTimeRange));
      onSeek(targetTime);
    }
  };
  ...
  return (
    <div className="flex border-b border-[#0A018A] bg-[#1D006B]/90 hover:bg-[#1D006B] min-h-[90px] transition-colors group/track min-w-max">
      ...
      <div
        ref={trackLaneRef}
        onMouseDown={handleTrackLaneMouseDown}
        onDragOver={(e) => {
          e.preventDefault();
          setIsDragOver(true);
        }}
        ...
        className={`flex-shrink-0 relative min-h-[90px] bg-[#1D006B]/50 transition-colors ${
