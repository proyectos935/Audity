...
export default function App() {
  ...
  // Equalizer (EQ) & Effects Modal State
  const [currentEQPreset, setCurrentEQPreset] = useState<string>('none');
  ...
  useEffect(() => {
    ...
    // Dynamic project duration strictly matching longest clip end time (or 0 if empty)
    const realDuration = max > 0 ? Number(max.toFixed(2)) : 0;
    setTotalDuration(realDuration);

    // If playhead was beyond the new duration (e.g. after deleting a long clip), reset to 0
    setPlayheadTime((prev) => (prev > realDuration ? 0 : prev));
  ...
  // Equalizer (EQ) Handler
  const handleSelectEQPreset = (presetId: string) => {
    setCurrentEQPreset(presetId);
    playerRef.current.setEQPreset(presetId, tracks, audioBufferMapRef.current, totalDuration);
    const presetObj = EQ_PRESETS.find((p) => p.id === presetId);
    if (presetId === 'none') {
      showToast('Ecualizador desactivado (Audio Plano)', 'info');
    } else if (presetObj) {
      showToast(`¡Efecto '${presetObj.name}' activado en la mezcla!`, 'success');
    }
  };

  // Player Handlers
  const handlePlay = () => {
    const playMax = playheadTime >= totalDuration && totalDuration > 0 ? playheadTime + 30 : totalDuration;
    playerRef.current.play(tracks, audioBufferMapRef.current, playMax, currentEQPreset);
    setIsPlaying(true);
  };
  ...
  const handleSeek = (time: number) => {
    ...
    const playMax = targetTime >= totalDuration && totalDuration > 0 ? targetTime + 30 : totalDuration;
    playerRef.current.seek(targetTime, tracks, audioBufferMapRef.current, playMax, currentEQPreset);
  ...
  const handleToggleRecord = async () => {
    if (isRecording) {
      ...
      try {
        ...
        // Determine clip start time (where playhead / Aguja currently is)
        const clipStart = playheadTime;
  ...
  const handleSaveProject = async () => {
    try {
      const blob = await saveProjectToALI(projectTitle, resources, tracks, totalDuration, currentEQPreset);
  ...
  const handleLoadProject = async (file: File) => {
    try {
      ...
      setCurrentEQPreset(loaded.masterEQPreset || 'none');
      playerRef.current.setEQPreset(loaded.masterEQPreset || 'none');
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-[#1D006B] font-sans overflow-hidden text-white">
      {/* 1. Header Minimalista con Botón EQ cerca de Guardar .ALI */}
      <Header
